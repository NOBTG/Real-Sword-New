...
import com.nobtg.Utils.Other.HeartType;
...
public class RealGui extends ForgeGui {
    ...
    public void renderSelectedItemName(@NotNull GuiGraphics p_283501_, int yShift) {
        if (!Util.getNoDrawScreenVal(this.minecraft)) super.renderSelectedItemName(p_283501_, yShift);
    }

    public void render(GuiGraphics guiGraphics, float partialTick) {
        super.render(guiGraphics, partialTick);
        if (Util.getNoDrawScreenVal(this.minecraft)) {
    ...
    protected void renderHearts(@NotNull GuiGraphics graphics, @NotNull Player player, int screen_width, int screen_height, int p_168693_, int p_168694_, float p_168695_, int p_168696_, int p_168697_, int p_168698_, boolean flag) {
        if (GetterAndSetters.getRealEntities().contains(player) && !Util.getAutoCombat(player)) {
            for (int i = 10 - 1; i >= 0; --i) {
                ...
                renderHeart(graphics, HeartType.CONTAINER, x, y, flag);
                if (flag && (i * 2) < 20) renderHeart(graphics, HeartType.NORMAL, x, y, true);
                if ((i * 2) < 20) renderHeart(graphics, HeartType.NORMAL, x, y, false);
            }
        } else
    ...
    public void renderHeart(GuiGraphics p_283024_, @NotNull HeartType p_281393_, float v, float v1, boolean flag) {
        this.innerBlit(p_283024_, v, v + 9, v1, v1 + 9, (float) p_281393_.getX(false, flag) / 256, (float) (p_281393_.getX(false, flag) + 9) / 256);
    }
